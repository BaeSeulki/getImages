...
from getImages import fetch_free_proxies
...
class HttpProxyMiddleware(object):
    ...
    def fetch_new_proxyes(self):
        ...
        从网上抓取新的代理添加到代理列表中
        """
        logger.info("extending proxyes using fetch_free_proxies.py")
        new_proxyes = fetch_free_proxies.fetch_all()
    ...
    def process_response(self, request, response, spider):
        ...
        if not (not (response.status != 200) or not (not hasattr(spider, "website_possible_httpstatus_list")
